...
class MainView:
    ...
    - Registrar ventas relacionando un usuario con un producto (Semana 15).
    ...
    Semana 15: se incorpora la sección de Ventas con el flujo de manejo de eventos:
    command= → callback → RestauranteServicio → persistencia → respuesta visual.
    Se utilizan íconos y el logotipo del sistema desde la carpeta assets/.
    """
    ...
    def __init__(self, root, servicio, al_cerrar_sesion):
        ...
        # Semana 15: referencias para la sección de ventas
        self.tabla_ventas = None
        self.combo_usuario_venta = None
        self.combo_producto_venta = None
        self.etiqueta_estado_ventas = None
        self.logo_imagen = None  # Referencia para evitar recolección de basura

        # Semana 15: referencias a los íconos cargados desde assets/
        self.iconos = {}
    ...
    # ================================================================
    # Carga de íconos desde assets/ (Semana 15)
    # ================================================================

    def _cargar_iconos(self):
        """
        Carga los íconos desde la carpeta assets/ para usarlos en los botones
        de navegación y la barra superior. Si algún ícono no se encuentra,
        se omite sin generar error.
        """
        ...
        nombres_iconos = {
            ...
            "inicio": "icono_inicio.png",
            "cerrar_sesion": "icono_cerrar_sesion.png"
        }

        for clave, archivo in nombres_iconos.items():
            try:
                ...
                if os.path.exists(ruta):
                    ...
                    ancho = imagen.width()
                    alto = imagen.height()
                    # Redimensionar a aproximadamente 20px para botones
                    factor = max(1, max(ancho, alto) // 20)
                    self.iconos[clave] = imagen.subsample(factor, factor)
            except Exception as e:
                print(f"No se pudo cargar el ícono '{archivo}': {e}")
    ...
    def _crear_barra_superior(self, usuario):
        """Crea la barra superior con logo, título, info del usuario y botón de cerrar sesión."""
        barra_superior = tk.Frame(self.frame, bg="#E74C3C", height=60)
        barra_superior.pack(fill="x")
        barra_superior.pack_propagate(False)

        # Contenedor izquierdo para logo + título
        frame_titulo = tk.Frame(barra_superior, bg="#E74C3C")
        frame_titulo.pack(side="left", padx=10)

        # Cargar logo en la barra superior
        self._cargar_logo_barra(frame_titulo)
        ...
        # Botón de cerrar sesión (con ícono desde assets/ si está disponible)
        btn_cerrar_config = {
            "text": "Cerrar Sesión",
            "font": ("Arial", 9, "bold"),
            "bg": "#C0392B",
        ...
        if "cerrar_sesion" in self.iconos:
            btn_cerrar_config["image"] = self.iconos["cerrar_sesion"]
        btn_cerrar = tk.Button(barra_superior, **btn_cerrar_config)
        btn_cerrar.pack(side="right", padx=10, pady=15)

    def _cargar_logo_barra(self, contenedor):
        """
        Carga el logotipo del restaurante en la barra superior desde assets/.
        ...
        try:
            ...
            if os.path.exists(ruta_assets):
                ...
                factor = max(1, max(ancho, alto) // 40)
            ...
            print(f"No se pudo cargar el logo en la barra: {e}")
    ...
    def _crear_panel_navegacion(self, contenedor_padre):
        ...
        Semana 15: los botones incorporan íconos cargados desde assets/.
        """
        panel_botones = tk.Frame(contenedor_padre, bg="#2C3E50")
        panel_botones.pack(fill="x", pady=(0, 10))

        # Botón Productos (con ícono desde assets/)
        btn_productos_config = {
            "text": " Productos",
            "font": ("Arial", 11, "bold"),
            "bg": "#27AE60",
            "fg": "white",
        ...
        if "productos" in self.iconos:
            btn_productos_config["image"] = self.iconos["productos"]
        tk.Button(panel_botones, **btn_productos_config).pack(side="left", padx=(0, 10), ipady=8)

        # Botón Usuarios (con ícono desde assets/)
        btn_usuarios_config = {
            "text": " Usuarios",
            "font": ("Arial", 11, "bold"),
            "bg": "#2980B9",
        ...
        if "usuarios" in self.iconos:
            btn_usuarios_config["image"] = self.iconos["usuarios"]
        tk.Button(panel_botones, **btn_usuarios_config).pack(side="left", padx=(0, 10), ipady=8)

        # Semana 15: botón de navegación a la sección de Ventas (con ícono desde assets/)
        btn_ventas_config = {
            "text": " Ventas",
            "font": ("Arial", 11, "bold"),
            "bg": "#E67E22",
            ...
            "cursor": "hand2",
            "width": 15,
            "command": self._mostrar_seccion_ventas,
            "compound": "left"
        }
        if "ventas" in self.iconos:
            btn_ventas_config["image"] = self.iconos["ventas"]
        tk.Button(panel_botones, **btn_ventas_config).pack(side="left", padx=(0, 10), ipady=8)

        # Botón Inicio (con ícono desde assets/)
        btn_inicio_config = {
            "text": " Inicio",
            "font": ("Arial", 11, "bold"),
            "bg": "#8E44AD",
        ...
        if "inicio" in self.iconos:
            btn_inicio_config["image"] = self.iconos["inicio"]
        tk.Button(panel_botones, **btn_inicio_config).pack(side="left", padx=(0, 10), ipady=8)
    ...
    def _mostrar_bienvenida(self, usuario):
        ...
        cantidad_ventas = self.servicio.obtener_cantidad_ventas()
        ...
        # Semana 15: mostrar cantidad de ventas en el resumen
        tk.Label(
            resumen,
            text=f"🧾 Ventas registradas: {cantidad_ventas}",
            font=("Arial", 12),
            fg="#E67E22",
            bg="#34495E"
        ).pack(anchor="w", pady=3)
    ...
    # ================================================================
    # Sección de VENTAS (Semana 15 - Manejo de eventos)
    # ================================================================

    def _mostrar_seccion_ventas(self):
        """
        Muestra la sección de registro de ventas.
        Permite seleccionar un usuario y un producto existentes para
        registrar una venta mediante el flujo de manejo de eventos:

        USUARIO (acción) → BOTÓN (command=) → CALLBACK (_callback_registrar_venta)
        → RestauranteServicio (validar y registrar) → PERSISTENCIA (ventas.json)
        → RESPUESTA VISUAL (tabla actualizada + mensaje).

        Los Combobox muestran la lista de usuarios y productos disponibles.
        El botón "Registrar venta" usa command=self._callback_registrar_venta.
        """
        self._limpiar_area_contenido()
        ...
        # ---- CONTENEDOR DEL FORMULARIO DE VENTA (LabelFrame) ----
        frame_formulario_venta = tk.LabelFrame(
            self.area_contenido,
            text=" Nueva Venta ",
            font=("Arial", 11, "bold"),
            fg="#ECF0F1",
        ...
        frame_formulario_venta.pack(fill="x", pady=(0, 10))

        # Fila 0: Selección de usuario
        tk.Label(
            frame_formulario_venta, text="👤 Usuario:",
            font=("Arial", 11, "bold"),
            fg="#ECF0F1", bg="#2C3E50"
        ...
        opciones_usuarios = [f"{u.id} - {u.nombre_completo} ({u.rol})" for u in usuarios]

        self.combo_usuario_venta = ttk.Combobox(
            frame_formulario_venta,
            values=opciones_usuarios,
            font=("Arial", 10),
            width=40,
            state="readonly"
        )
        self.combo_usuario_venta.grid(row=0, column=1, sticky="w", padx=(0, 20), pady=8, ipady=3)
        ...
        tk.Label(
            frame_formulario_venta, text="📦 Producto:",
            font=("Arial", 11, "bold"),
            fg="#ECF0F1", bg="#2C3E50"
        ...
        productos = self.servicio.obtener_productos()
        opciones_productos = [
            f"{p.id} - {p.nombre} (${p.precio:.2f}) - Stock: {p.cantidad}"
            for p in productos
        ]
        ...
        self.combo_producto_venta.grid(row=1, column=1, sticky="w", padx=(0, 20), pady=8, ipady=3)

        if opciones_productos:
            self.combo_producto_venta.current(0)

        # ---- BOTÓN REGISTRAR VENTA ----
        # Semana 15: el botón utiliza command= para asociar el callback.
        # NO se usa command=self._callback_registrar_venta() (con paréntesis).
        # Se usa command=self._callback_registrar_venta (sin paréntesis),
        # lo que permite que Tkinter ejecute el callback cuando el usuario
        # haga clic en el botón.
        frame_boton_venta = tk.Frame(self.area_contenido, bg="#34495E")
        frame_boton_venta.pack(fill="x", pady=(0, 10))
        ...
        # ---- CONTENEDOR DE LA TABLA DE VENTAS (LabelFrame) ----
        frame_tabla_ventas = tk.LabelFrame(
            self.area_contenido,
        ...
        frame_tabla_ventas.pack(fill="both", expand=True, pady=(0, 5))

        # Crear tabla de ventas con Treeview
        columnas_ventas = ("ID", "Usuario", "Producto", "Precio", "Fecha")
        ...
        # Configurar encabezados de la tabla de ventas
        ...
        self.tabla_ventas.column("Usuario", width=180, anchor="w")
        self.tabla_ventas.column("Producto", width=180, anchor="w")
        self.tabla_ventas.column("Precio", width=90, anchor="center")
        self.tabla_ventas.column("Fecha", width=160, anchor="center")

        # Scrollbar vertical para la tabla de ventas
        scrollbar_ventas = ttk.Scrollbar(frame_tabla_ventas, orient="vertical",
                                          command=self.tabla_ventas.yview)
        self.tabla_ventas.configure(yscrollcommand=scrollbar_ventas.set)

        self.tabla_ventas.pack(side="left", fill="both", expand=True)
        scrollbar_ventas.pack(side="right", fill="y")

        # Cargar las ventas registradas en la tabla
        self._actualizar_tabla_ventas()

        # ---- BARRA DE ESTADO DE VENTAS ----
        frame_estado_ventas = tk.Frame(self.area_contenido, bg="#2C3E50", pady=5)
        frame_estado_ventas.pack(fill="x")

        self.etiqueta_estado_ventas = tk.Label(
            frame_estado_ventas,
            text=f"Total de ventas: {self.servicio.obtener_cantidad_ventas()}",
            font=("Arial", 10),
            fg="#BDC3C7",
            bg="#2C3E50"
        )
        self.etiqueta_estado_ventas.pack(side="left")

    # ================================================================
    # Callback de venta (Semana 15 - Manejo de eventos)
    # ================================================================

    def _callback_registrar_venta(self):
        """
        Callback ejecutado cuando el usuario presiona el botón "Registrar Venta".

        Este método implementa el flujo fundamental de manejo de eventos:
        1. Obtiene las selecciones realizadas en la interfaz (Combobox).
        2. Extrae los IDs del usuario y producto seleccionados.
        3. Delega la operación de registro a RestauranteServicio.
        4. Actualiza la vista con el resultado de la operación.
        5. Comunica al usuario el resultado mediante mensajes visuales.

        El callback coordina la interacción pero NO contiene la lógica
        de negocio ni accede directamente a los archivos JSON. Toda la
        validación y persistencia se realiza dentro de RestauranteServicio.
        """
        # Paso 1: Obtener las selecciones de la interfaz
        seleccion_usuario = self.combo_usuario_venta.get()
        seleccion_producto = self.combo_producto_venta.get()

        # Validar que se hayan realizado selecciones
        if not seleccion_usuario:
            messagebox.showwarning(
                "Selección requerida",
                "Debe seleccionar un usuario para registrar la venta."
            )
            return
        ...
        # Paso 2: Extraer los IDs de las selecciones (formato: "ID - Nombre...")
        try:
            id_usuario = int(seleccion_usuario.split(" - ")[0])
            id_producto = int(seleccion_producto.split(" - ")[0])
        ...
        # Paso 3: Delegar la operación al servicio
        exito, mensaje = self.servicio.registrar_venta(id_usuario, id_producto)

        # Paso 4 y 5: Actualizar la vista y comunicar el resultado
        if exito:
            # Actualizar la tabla de ventas con la nueva venta
            self._actualizar_tabla_ventas()
            # Actualizar los productos en el Combobox (el stock cambió)
            self._actualizar_combo_productos()
            # Mostrar mensaje de éxito en la barra de estado
            self._mostrar_estado_ventas(mensaje, "#2ECC71")
            messagebox.showinfo("Venta registrada", mensaje)
        else:
            # Mostrar mensaje de error en la barra de estado
            self._mostrar_estado_ventas(mensaje, "#E74C3C")
            messagebox.showwarning("Error al registrar venta", mensaje)

    # ================================================================
    # Métodos auxiliares de la sección de ventas
    # ================================================================

    def _actualizar_tabla_ventas(self):
        """
        Actualiza la tabla Treeview de ventas con los datos actuales del servicio.
        Limpia las filas existentes y las recarga desde RestauranteServicio.
        """
        ...
        # Insertar los datos actualizados desde el servicio
        ...
        for venta in ventas:
            self.tabla_ventas.insert("", "end", values=(
                ...
                f"${venta.precio_producto:.2f}",
                venta.fecha
            ))

        # Actualizar el contador en la barra de estado de ventas
        if self.etiqueta_estado_ventas:
            self.etiqueta_estado_ventas.config(
                text=f"Total de ventas: {self.servicio.obtener_cantidad_ventas()}"
            )

    def _actualizar_combo_productos(self):
        """
        Actualiza las opciones del Combobox de productos después de registrar
        una venta (ya que el stock del producto seleccionado se reduce).
        """
        if not self.combo_producto_venta:
            return
        ...
        self.combo_producto_venta["values"] = opciones_productos

        if opciones_productos:
            self.combo_producto_venta.current(0)

    def _mostrar_estado_ventas(self, mensaje, color="#BDC3C7"):
        """
        Actualiza el mensaje de la barra de estado de la sección de ventas.
        ...
        """
        if self.etiqueta_estado_ventas:
            self.etiqueta_estado_ventas.config(text=mensaje, fg=color)
    ...
    def _limpiar_area_contenido(self):
        ...
        self.tabla_productos = None
        self.etiqueta_estado = None
        # Resetear referencias a componentes de la sección de ventas
        self.tabla_ventas = None
        self.combo_usuario_venta = None
        self.combo_producto_venta = None
        self.etiqueta_estado_ventas = None
